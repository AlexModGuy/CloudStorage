...
public class VillagerHatLayer<T extends LivingEntity, M extends EntityModel<T> & HeadedModel> extends RenderLayer<T, M> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int packedLight, T entity, float f1, float f2, float f3, float f4, float f5, float f6) {
        if(entity instanceof Villager && ((Villager)entity).getVillagerData().getProfession() == CSVillagerRegistry.BALLOON_SALESMAN || entity instanceof ZombieVillager && ((ZombieVillager)entity).getVillagerData().getProfession() == CSVillagerRegistry.BALLOON_SALESMAN){
            if(!entity.isBaby()){
                poseStack.pushPose();
